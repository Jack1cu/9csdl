...
// Truy vấn SQL để thống kê theo tháng
$sql = "
    SELECT 
        DATE_FORMAT(d.ngaymua, '%Y-%m') AS thang,
        SUM(d.tongtien) AS tongtien,
        SUM(c.soluong) AS soluongsp
    FROM 
        donhang d
    JOIN 
        chitietdonhang c ON d.id = c.iddonhang
    GROUP BY 
        thang
    ORDER BY 
        thang;
...
while ($row = mysqli_fetch_array($ketqua)) {
    ...
    echo "<td>".number_format($row['tongtien'], 0, ',', '.')."đ</td>"; // Định dạng tiền
