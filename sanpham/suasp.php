...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['submit'])) {
        ...
        $sql_old = "SELECT hinhanh FROM sanpham WHERE id=" . $_GET['id'];
        $result_old = mysqli_query($conn, $sql_old);
        $row_old = mysqli_fetch_assoc($result_old);

        // Nếu có ảnh cũ, gán cho biến hinhanh
        if ($row_old) {
            $hinhanh = $row_old['hinhanh'];
        }
		
        // Kiểm tra và xử lý file upload
        if (isset($_FILES['uploadFile']) && $_FILES['uploadFile']['name'] != NULL) {
            // Kiểm tra file upload có phải là ảnh không
            $allowed_types = ["image/jpeg", "image/png", "image/gif"];
            if (in_array($_FILES['uploadFile']['type'], $allowed_types)) {
                $path = "img/"; // Thư mục lưu ảnh
                ...
                echo "Chỉ hỗ trợ định dạng ảnh JPEG, PNG và GIF.";
            }
        }
        ...
        $sql = "UPDATE sanpham SET tensp=?, giasp=?, giamgia=?, mota=?, hinhanh=?, soluong=?, size=?, mau=?, iddanhmuc=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $sql);

        // Kiểm tra việc chuẩn bị câu lệnh
        if ($stmt) {
            // Bind các tham số
            mysqli_stmt_bind_param($stmt, 'sdissisiii', $tensp, $giasp, $giamgia, $mota, $hinhanh, $soluong, $size, $mau, $iddanhmuc, $_GET['id']);
            // Thực thi câu lệnh
            ...
            echo "Không thể chuẩn bị câu lệnh: " . mysqli_error($conn);
        }
